...
* Solve Quadratic Residues (x^2=a(mod n) which (a,n)=1)
* author: cylixstar
* date: 2010-10-09
...
void getSqr(int r,int p,int& r1,int& r2){
    ...
    while(t!=1){
        for(i=1,dt=longMod(t,t,p);dt!=1;i++)
            dt =longMod(dt,dt,p);
        for(j=M-i-1;j>0;j--)c=longMod(c,c,p);
        R = longMod(R,c,p);
        c = longMod(c,c,p);
        t = longMod(t,c,p);
        M=i;
...
void solve(int r,int n,VI& res){
    ...
    for(i=0;i<fac.size();i++){
        tmp.clear();
        p = 1;
        for(j=0;j<fac[i].second;j++)p*=fac[i].first;
        if(fac[i].first==2)
            getPowerSol2(r,fac[0].second,tmp);
        else{
            if(pm(r,(fac[i].first - 1)>>1,fac[i].first)!=1)return;
